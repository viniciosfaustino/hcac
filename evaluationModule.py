...
def get_fscore(hcac: HCAC):
    ...
    for i in range(hcac.dataset.size):
        c_size = np.sum(hcac.cluster.classes_per_cluster[i])
        for j in range(hcac.dataset.number_of_classes):
            f = 0
            k_size = hcac.cluster.classes_per_cluster[hcac.dataset.size - 2][j]
    ...
    for i in range(hcac.dataset.number_of_classes):
        score += f_max[i] * hcac.cluster.classes_per_cluster[hcac.dataset.size - 2][i] / hcac.dataset.size
    return score
